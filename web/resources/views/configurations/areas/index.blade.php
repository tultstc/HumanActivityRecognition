...
@section('content')
    <div>
        <a href="{{ route('configurations.areas.create') }}" class="btn btn-primary float-start mt-1">Add Area</a>
    </div>
        ...
        <tbody>
            @foreach ($areas as $area)
                <tr>
                    ...
                    <td>
                        {{-- @can('update area') --}}
                        <a href="{{ url('configurations/areas/' . $area->id . '/edit') }}" class="btn btn-success">Edit</a>
                        {{-- @endcan --}}

                        {{-- @can('delete area') --}}
                        <a href="{{ url('configurations/areas/' . $area->id . '/delete') }}"
