...
optim_wrapper = dict(
    ...
    optimizer=dict(lr=0.001, type='Adam', weight_decay=0.0003))
param_scheduler = [
    dict(
        T_max=24,
        by_epoch=True,
...
test_pipeline = [
    dict(clip_len=50, num_clips=1, test_mode=True, type='UniformSampleFrames'),
    dict(type='PoseDecode'),
    dict(allow_imgpad=True, hw_ratio=1.0, type='PoseCompact'),
    dict(scale=(
        -1,
...
train_dataloader = dict(
    ...
    dataset=dict(
        dataset=dict(
            ann_file='ntu_custom_dataset.pkl',
            pipeline=[
                dict(clip_len=50, num_clips=1, type='UniformSampleFrames'),
                dict(type='PoseDecode'),
                dict(allow_imgpad=True, hw_ratio=1.0, type='PoseCompact'),
                dict(scale=(
                    -1,
...
work_dir = 'slowonly_r50_8xb16-u48-240e_ntu60-xsub-keypoint'
